    ...
    return {
        ...
        checkForWinner: () => {
            ...
            if (tileValues[0] === users[activePlayer]) {
                if (
                    (tileValues[1] === users[activePlayer] &&
                        tileValues[2] === users[activePlayer]) ||
                    (tileValues[3] === users[activePlayer] &&
                        tileValues[6] === users[activePlayer])
                )
                    return true;
            }
            if (tileValues[8] === users[activePlayer]) {
                if (
                    (tileValues[7] === users[activePlayer] &&
                        tileValues[6] === users[activePlayer]) ||
                    (tileValues[5] === users[activePlayer] &&
                        tileValues[2] === users[activePlayer])
                )
                    return true;
            }
            if (tileValues[4] === users[activePlayer]) {
                for (let upper = 0, last = 8; upper < 4; upper++, last--) {
                    if (
                        tileValues[upper] === users[activePlayer] &&
                        tileValues[last] === users[activePlayer]
                    )
                        return true;
                }
            }
...
let UIController = (function() {
    ...
    return {
        ...
        updateStatusText: (textValue) =>
            (document.getElementById(textArea).innerHTML = textValue),
...
let controller = (function(playerC, uiC) {
    ...
    function disableGame() {
        myTiles.forEach((el) =>
            el.removeEventListener("click", checkAndChangeTile)
        );
    }
    ...
    function checkAndChangeTile() {
        if (playerC.verifySelection(this.id)) {
            document
                ...
                .removeEventListener("click", this.handleTileSelection);
    ...
    function setPlayerSwap() {
        if (playerC.checkForWinner()) {
            disableGame();
            uiC.updateStatusText(
                `${playerC.getCurrPlayer()} you won congratulations!`
            );
        } else {
